...
vec2 opFluid(vec2 q, vec4 P0){
  ...
  { name:'CA warp', fn:'opCA', deps:[],
    params:[["Amount",-4,4,0.01,1],["Source",0,1,1,1,["cells","trails"]],["Mode",0,1,1,0,["gradient","value"]],["Scale",0.25,3,0.01,1]],
    glsl:`vec2 opCA(vec2 q, vec4 P0){
  if(uCAOn < 0.5) return q;
  float amt=P0.x, chan=P0.y, mode=P0.z, scl=P0.w;
  float ca=uCanvas.x/uCanvas.y;
  vec2 uv=clamp((q/vec2(ca,1.0))*scl + uCenter, 0.0, 1.0);
  float tx=uCATexel;
  if(mode < 0.5){                                  // ride the edges of the pattern
    vec4 l=texture2D(uCAField, clamp(uv-vec2(tx,0.0),0.0,1.0));
    vec4 r=texture2D(uCAField, clamp(uv+vec2(tx,0.0),0.0,1.0));
    vec4 b=texture2D(uCAField, clamp(uv-vec2(0.0,tx),0.0,1.0));
    vec4 t=texture2D(uCAField, clamp(uv+vec2(0.0,tx),0.0,1.0));
    vec2 g = (chan<0.5) ? vec2(r.r-l.r, t.r-b.r) : vec2(r.g-l.g, t.g-b.g);
    return q + g*amt*0.35;
  }
  vec4 f=texture2D(uCAField, uv);                  // displace by the living cells
  float v = (chan<0.5) ? f.r : f.g;
